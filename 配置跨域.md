...
可以看出， ```The 'Access-Control-Allow-Origin' header contains multiple values 'http://localhost, *'``` 和 ```The 'Access-Control-Allow-Origin' header contains multiple values '*, *'``` 里 ```Access-Control-Allow-Origin``` 头包含的值是网关配置的 ```Access-Control-Allow-Origin``` 和 nginx 配置的  ```Access-Control-Allow-Origin```
